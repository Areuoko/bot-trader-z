...
# Symbols & Timeframe
# ─────────────────────────────────────────────
SYMBOLS = ["XAUUSD_o", "XAGUSD_o", "EURUSD_o"]
SYMBOL = SYMBOLS[0]   # نگه‌داشته شده برای سازگاری با کدهای قدیمی/تست‌های مستقل هر فایل
...
# Liquidity Sweep
SWEEP_BUFFER_PTS = 2    # بافر پیش‌فرض (fallback) - فقط برای نمادهایی که در دیکشنری زیر نیستند

# دیباگ مهم: SWEEP_BUFFER_PTS یک عدد مطلق قیمتی است، نه "پوینت" واقعی بروکر.
# چون رنج قیمتی طلا (~4000)، نقره (~35-50) و یورودلار (~1.08) کاملاً متفاوتند،
# یک بافر ثابت برای هر سه غیرممکنه (روی نقره/یورو صفر سیگنال می‌دهد).
# این مقادیر نقطه‌ی شروع‌اند؛ با backtest_signal_frequency.py کالیبره‌شان کن.
SWEEP_BUFFER_BY_SYMBOL = {
    "XAUUSD_o": 2.0,      # طلا - همان مقدار قبلی
    "XAGUSD_o": 0.02,     # نقره - نسبت به رنج قیمتی نقره
    "EURUSD_o": 0.0005,   # یورودلار - معادل تقریبی 5 پیپ
}
...
NEWS_RELEVANT_CURRENCIES = ["USD"]          # هر سه نماد (XAUUSD/XAGUSD/EURUSD) به دلار وابسته‌اند
...
MAX_OPEN_POSITIONS_TOTAL = 3         # سقف کل پوزیشن‌های باز روی همه‌ی نمادها
MAX_OPEN_POSITIONS_PER_SYMBOL = 1    # حداکثر یک پوزیشن باز روی هر نماد
